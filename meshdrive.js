...
module.exports.meshdrive = function (parent) {
    ...
    function log(m) {
        try { obj.debug('PLUGIN', 'Mesh Drive', m); } catch (e) {}
        try { console.log('PLUGIN: Mesh Drive: ' + m); } catch (e) {}
    }
    ...
    function dbGet(id) {
        return new Promise(function(resolve) {
            var db = obj.meshServer && (obj.meshServer.db || (obj.meshServer.webserver && obj.meshServer.webserver.db));
    ...
    function tseq(a, b) {
        ...
        var ab = Buffer.from(a), bb = Buffer.from(b);
        if (ab.length !== bb.length) return false;
        try { return crypto.timingSafeEqual(ab, bb); } catch (e) { return false; }
    }
    function hashLen(h) {
        try { var b = Buffer.from(String(h || ''), 'base64'); if (b && b.length > 0) return b.length; } catch (e) {}
        return 64;
    }
    function pbkdf2(pw, salt, stored) {
        return new Promise(function(resolve) {
            try {
                crypto.pbkdf2(pw, salt, cfg.passwordIterations, hashLen(stored), 'sha384', function(err, h) {
                    if (err) { log('pbkdf2 error: ' + err); return resolve(null); }
                    resolve(h.toString('base64'));
                });
    ...
    function full(u, rel) {
        var r = userRoot(u), clean = path.normalize('/' + rel).replace(/^([/\\])+/, '');
        var p = path.resolve(path.join(r, clean));
        if (p !== r && p.indexOf(r + path.sep) !== 0) return null;
        return p;
    }
    ...
    async function dav(req, res) {
        var u = await auth(req, res); if (!u) return;
