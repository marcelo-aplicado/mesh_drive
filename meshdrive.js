...
module.exports.meshdrive = function (parent) {
  ...
  function defaultShares(){ return { shares:[{ name:'Contatos', path:'contatos', readUsers:['*'], writeUsers:['marcelo'], readGroups:[], writeGroups:['TI'], anonymousAccess:'read', icon:'contact' }] }; }
  ...
  function iconMode(v){ v=String(v||'folder').toLowerCase(); return (v==='contact'||v==='public'||v==='folder')?v:'folder'; }
  function normalizeShare(s){
    ...
    return { name:safe(s.name||'Share'), path:safePath(s.path||('shares/'+safe(s.name||'Share'))), readUsers, writeUsers, readGroups, writeGroups, anonymousAccess:anonMode(s.anonymousAccess), icon:iconMode(s.icon) };
  }
  ...
  const meshDriveIconBase64 = { contact:'', public:'' };
  function writeHiddenIconFile(dir,name,b64){ try{ const f=path.join(dir,name); if(!fs.existsSync(f)) fs.writeFileSync(f,Buffer.from(b64,'base64')); }catch(e){} }
  function ensureShareIcon(dir,share){
    try{
      const icon=iconMode(share.icon);
      const ini=path.join(dir,'desktop.ini');
      if(icon==='folder'){ if(fs.existsSync(ini)){ try{fs.rmSync(ini,{force:true});}catch(e){} } return; }
      const iconName='.meshdrive-'+icon+'.ico';
      writeHiddenIconFile(dir,iconName,meshDriveIconBase64[icon]);
      const content='[.ShellClassInfo]\r\nIconFile='+iconName+'\r\nIconIndex=0\r\nConfirmFileOp=0\r\nInfoTip=Mesh Drive - '+share.name+'\r\n';
      fs.writeFileSync(ini,content,'utf8');
    }catch(e){}
  }
  function isMeshDriveInternalFile(n){ n=String(n||'').toLowerCase(); return n==='desktop.ini'||n.indexOf('.meshdrive-')===0; }
  function shareRoot(u,share){ const root=tenantRoot(u),rel=safePath(share.path||('shares/'+share.name)),p=path.resolve(path.join(root,rel)); if(p!==root&&p.indexOf(root+path.sep)!==0)return null; mkdir(p); ensureShareIcon(p,share); return p; }
  ...
  function rootPropfind(req,res,u){
    ...
    if(root && fs.existsSync(root)){
      out += propReal(root,'/');
      if(depth !== '0'){
        fs.readdirSync(root).filter(n=>!isMeshDriveInternalFile(n)).forEach(n=>{ out += propReal(path.join(root,n),'/'+n); });
  ...
  function handleRealDav(req,res,target,u){ switch((req.method||'GET').toUpperCase()){
    case 'PROPFIND': { if(!fs.existsSync(target.path)){res.writeHead(404);return res.end();} let out=propReal(target.path,target.rel); const depth=req.headers.depth||'1',st=fs.statSync(target.path); if(depth!=='0'&&st.isDirectory())fs.readdirSync(target.path).filter(n=>!isMeshDriveInternalFile(n)).forEach(n=>{out+=propReal(path.join(target.path,n),path.posix.join(target.rel,n));}); return xml(res,207,'<?xml version="1.0" encoding="utf-8"?><D:multistatus xmlns:D="DAV:">'+out+'</D:multistatus>'); }
