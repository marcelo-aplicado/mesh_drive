...
module.exports.meshdrive = function (parent) {
    ...
    function log(message) {
        try { obj.debug('PLUGIN', 'Mesh Drive', message); } catch (e) {}
        try { console.log('PLUGIN: Mesh Drive: ' + message); } catch (e) {}
    }
    ...
    function getDb() {
        return obj.meshServer && (obj.meshServer.db || (obj.meshServer.webserver && obj.meshServer.webserver.db));
    }
    ...
    function hashPassword(passModule, password, salt) {
        return new Promise(function(resolve) {
            try {
                ...
                passModule.hash(password, salt, function() {
                    var args = Array.prototype.slice.call(arguments);
                    var last = args[args.length - 1];
                    if (Buffer.isBuffer(last)) return resolve(last.toString('hex'));
                    resolve(String(last || ''));
                });
            } catch (e) { resolve(null); }
        });
    ...
    async function findLocalUser(username) {
        ...
        var candidates = [
            'user//' + u,
            'user/' + cfg.meshDomainFolder + '/' + u,
            'user//user-' + u,
            'user/' + cfg.meshDomainFolder + '/user-' + u
        ];
        for (var i = 0; i < candidates.length; i++) {
            var doc = await dbGet(candidates[i]);
            if (doc) return { id: candidates[i], doc: doc, username: u };
        }
    ...
    async function validateLocalUser(username, password) {
        ...
        var found = await findLocalUser(username);
        var userDoc = found.doc;
        ...
        var passModule = loadPassModule();
        var computed = await hashPassword(passModule, password, salt);
    ...
    function nativeDomainRoot() {
        return path.resolve(path.join(cfg.meshFilesRoot, cfg.meshDomainFolder));
    }

    function userRoot(user) {
        var folder = cfg.userFolderPrefix + normalizeUsername(user.username || user.id || 'user');
        var root = path.join(nativeDomainRoot(), folder);
    ...
    function fullPath(user, rel) {
        var root = userRoot(user);
        var clean = path.normalize('/' + rel).replace(/^([/\\])+/, '');
        var p = path.resolve(path.join(root, clean));
        if (p !== root && p.indexOf(root + path.sep) !== 0) return null;
        return p;
    }

    function externalHref(rel) {
        var base = cfg.route.replace(/\/$/, '');
        var r = rel || '/';
        if (r.indexOf('/') !== 0) r = '/' + r;
        return base + encodeURI(r).replace(/#/g, '%23');
    }
    ...
    function propResponse(fsPath, rel) {
        var st = fs.statSync(fsPath);
        var isDir = st.isDirectory();
        var display = path.basename(fsPath) || '/';
        var href = externalHref(rel + (isDir && !rel.endsWith('/') ? '/' : ''));
        return '<D:response>' +
            ...
            '<D:getetag>"' + st.size + '-' + Number(st.mtimeMs).toString(16) + '"</D:getetag>' +
            '</D:prop><D:status>HTTP/1.1 200 OK</D:status></D:propstat>' +
            '</D:response>';
    }

    function sendXml(res, code, body, headers) {
        res.writeHead(code, Object.assign({ 'Content-Type': 'application/xml; charset=utf-8' }, headers || {}));
        res.end(body);
    }
    ...
    function copyRecursive(src, dest) {
        ...
        if (st.isDirectory()) {
            ensureDir(dest);
            fs.readdirSync(src).forEach(function(f) { copyRecursive(path.join(src, f), path.join(dest, f)); });
        } else {
    ...
    async function handler(req, res) {
        ...
        var rel = requestPath(req);
        var fp = fullPath(user, rel);
        ...
        try {
            switch ((req.method || 'GET').toUpperCase()) {
                ...
                case 'PROPFIND': {
                    ...
                    var responses = propResponse(fp, rel);
                    var stat = fs.statSync(fp);
                    if (depth !== '0' && stat.isDirectory()) {
                        fs.readdirSync(fp).forEach(function(name) {
                            var child = path.join(fp, name);
                            var childRel = path.posix.join(rel, name);
                            responses += propResponse(child, childRel);
                        });
                    }
                    sendXml(res, 207, '<?xml version="1.0" encoding="utf-8"?><D:multistatus xmlns:D="DAV:">' + responses + '</D:multistatus>');
                    break;
                }
                case 'GET':
                case 'HEAD': {
                    if (!fs.existsSync(fp)) { res.writeHead(404); res.end(); return; }
                    var stat2 = fs.statSync(fp);
                    if (stat2.isDirectory()) { res.writeHead(403); res.end(); return; }
                    res.writeHead(200, { 'Content-Length': stat2.size });
                    if (req.method.toUpperCase() === 'HEAD') res.end(); else fs.createReadStream(fp).pipe(res);
                    break;
                }
                case 'PUT':
                    if (cfg.readOnly) return methodNotAllowed(res);
                    ensureDir(path.dirname(fp));
                    req.pipe(fs.createWriteStream(fp))
                        .on('finish', function() { res.writeHead(201); res.end(); })
                        .on('error', function() { res.writeHead(500); res.end(); });
                    break;
                case 'MKCOL':
                    if (cfg.readOnly) return methodNotAllowed(res);
                    if (fs.existsSync(fp)) { res.writeHead(405); res.end(); return; }
                    ensureDir(fp); res.writeHead(201); res.end();
                    break;
                case 'DELETE':
                    if (cfg.readOnly) return methodNotAllowed(res);
                    if (!fs.existsSync(fp)) { res.writeHead(404); res.end(); return; }
                    fs.rmSync(fp, { recursive: true, force: true }); res.writeHead(204); res.end();
                    break;
                case 'MOVE':
                case 'COPY': {
                    if (cfg.readOnly) return methodNotAllowed(res);
                    ...
                    if (!destHeader) { res.writeHead(400); res.end(); return; }
                    var destUrl = new URL(destHeader, cfg.publicUrl || 'https://mesh.aplicado.com.br/drive');
                    var destRel = decodeURIComponent(destUrl.pathname);
                    if (destRel.indexOf(cfg.route) === 0) destRel = destRel.substring(cfg.route.length) || '/';
                    var dest = fullPath(user, destRel);
                    if (!dest) { res.writeHead(403); res.end(); return; }
                    ensureDir(path.dirname(dest));
                    if (req.method.toUpperCase() === 'MOVE') fs.renameSync(fp, dest); else copyRecursive(fp, dest);
                    res.writeHead(201); res.end();
                    break;
                }
                case 'LOCK': {
                    var token = 'opaquelocktoken:' + crypto.randomUUID();
                    sendXml(res, 200, '<?xml version="1.0" encoding="utf-8"?><D:prop xmlns:D="DAV:"><D:lockdiscovery><D:activelock><D:locktype><D:write/></D:locktype><D:lockscope><D:exclusive/></D:lockscope><D:depth>infinity</D:depth><D:owner>Mesh Drive</D:owner><D:timeout>Second-3600</D:timeout><D:locktoken><D:href>' + token + '</D:href></D:locktoken></D:activelock></D:lockdiscovery></D:prop>', { 'Lock-Token': '<' + token + '>' });
                    break;
                }
                case 'UNLOCK':
                    ...
                    break;
                case 'PROPPATCH':
                    sendXml(res, 207, '<?xml version="1.0" encoding="utf-8"?><D:multistatus xmlns:D="DAV:"><D:response><D:href>' + xmlEscape(externalHref(rel)) + '</D:href><D:propstat><D:prop/><D:status>HTTP/1.1 200 OK</D:status></D:propstat></D:response></D:multistatus>');
                    break;
                default:
    ...
    obj.hook_setupHttpHandlers = function() {
        ...
        var app = findApp();
        if (!app) { log('Express app not found; cannot register route ' + cfg.route); return; }
        ensureDir(nativeDomainRoot());
        app.use(cfg.route, function(req, res) { handler(req, res); });
        log('registered route ' + cfg.route + ' -> ' + nativeDomainRoot() + '/' + cfg.userFolderPrefix + '<username>');
    };

    obj.server_startup = function() {
        log('loaded for ' + cfg.publicUrl + ', root=' + nativeDomainRoot());
    };
