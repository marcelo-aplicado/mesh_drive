...
module.exports.meshdrive = function (parent) {
  ...
  function parseVCard(txt,file){ const c={file:file||'',uid:'',fullName:'',firstName:'',lastName:'',organization:'',title:'',email:'',phone:'',mobile:'',note:''}; vcardUnfold(txt).forEach(l=>{ const u=String(l||'').toUpperCase(); if(u.indexOf('UID')===0)c.uid=vcardUnescape(vcardValue(l)); else if(u.indexOf('FN')===0)c.fullName=vcardUnescape(vcardValue(l)); else if(u.indexOf('N')===0){ const a=vcardValue(l).split(';'); c.lastName=vcardUnescape(a[0]||''); c.firstName=vcardUnescape(a[1]||''); } else if(u.indexOf('ORG')===0)c.organization=vcardUnescape(vcardValue(l)); else if(u.indexOf('TITLE')===0)c.title=vcardUnescape(vcardValue(l)); else if(u.indexOf('EMAIL')===0&&!c.email)c.email=vcardUnescape(vcardValue(l)); else if(u.indexOf('TEL')===0){ if((u.indexOf('CELL')>=0||u.indexOf('MOBILE')>=0) && !c.mobile)c.mobile=vcardUnescape(vcardValue(l)); else if(!c.phone)c.phone=vcardUnescape(vcardValue(l)); } else if(u.indexOf('NOTE')===0)c.note=vcardUnescape(vcardValue(l)); }); if(!c.fullName)c.fullName=((c.firstName+' '+c.lastName).trim()||c.email||c.phone||file||'Contato'); return c; }
  function contactFileName(c){ const base=safe((c.uid||c.fullName||c.email||crypto.randomUUID()).replace(/\.vcf$/i,'')); return base+'.vcf'; }
  function buildVCard(c){ const uid=c.uid||crypto.randomUUID(); const fn=(c.fullName||((c.firstName||'')+' '+(c.lastName||'')).trim()||c.email||'Contato'); const lines=['BEGIN:VCARD','VERSION:3.0','UID:'+vcardEscape(uid),'FN:'+vcardEscape(fn),'N:'+vcardEscape(c.lastName||'')+';'+vcardEscape(c.firstName||'')+';;;']; if(c.organization)lines.push('ORG:'+vcardEscape(c.organization)); if(c.title)lines.push('TITLE:'+vcardEscape(c.title)); if(c.email)lines.push('EMAIL;TYPE=INTERNET:'+vcardEscape(c.email)); if(c.phone)lines.push('TEL;TYPE=WORK,VOICE:'+vcardEscape(c.phone)); if(c.mobile)lines.push('TEL;TYPE=CELL:'+vcardEscape(c.mobile)); if(c.note)lines.push('NOTE:'+vcardEscape(c.note)); lines.push('END:VCARD'); return lines.join('\r\n')+'\r\n'; }
  function contactsHtml(ctx){ return '<!doctype html><html><head><meta charset="utf-8"><title>Mesh Drive - Contatos</title><style>body{font-family:Segoe UI,Arial,sans-serif;margin:24px;background:#f6f8fa;color:#24292f}.card{background:white;border:1px solid #d0d7de;border-radius:10px;padding:18px;margin-bottom:14px}.top{display:flex;gap:8px;align-items:center;margin-bottom:14px}button{padding:8px 12px;border-radius:6px;border:1px solid #1f6feb;background:#1f6feb;color:white;cursor:pointer}.secondary{background:#57606a;border-color:#57606a}.danger{background:#cf222e;border-color:#cf222e}input,textarea,select{box-sizing:border-box;padding:8px;border:1px solid #d0d7de;border-radius:6px}table{width:100%;border-collapse:collapse;background:white;border:1px solid #d0d7de;border-radius:10px;overflow:hidden}th,td{padding:8px;border-bottom:1px solid #d0d7de;text-align:left;font-size:13px}th{background:#f6f8fa}.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.grid label{display:block;font-size:12px;font-weight:600;margin:6px 0 4px}.small{color:#57606a;font-size:13px}</style></head><body><h1>Mesh Drive - Contatos CardDAV</h1><p class="small">Tenant: <b>'+x(tenantKey(ctx))+'</b> | Editor simples para contatos .vcf de compartilhamentos CardDAV.</p><div class="top"><select id="book"></select><input id="q" placeholder="Pesquisar..." style="flex:1"><button onclick="loadContacts()">Recarregar</button><button onclick="newContact()">Novo contato</button><button class="secondary" onclick="location.href=\'/meshdrive\'">Compartilhamentos</button></div><div id="list"></div><div id="editor" class="card" style="display:none"></div><script>let books=[],contacts=[],current=null;function esc(s){return String(s||\'\').replace(/[&<>\"]/g,c=>({\'&\':\'&amp;\',\'<\':\'&lt;\',\'>\':\'&gt;\',\'"\':\'&quot;\'}[c]))}async function init(){let r=await fetch(location.pathname+\'/list\');let j=await r.json();books=j.books||[];let b=document.getElementById(\'book\');b.innerHTML=books.map(x=>`<option>${esc(x.name)}</option>`).join(\'\');b.onchange=loadContacts;document.getElementById(\'q\').oninput=render;await loadContacts()}async function loadContacts(){let name=document.getElementById(\'book\').value||\'\';if(!name){document.getElementById(\'list\').innerHTML=\'<p>Nenhum compartilhamento CardDAV encontrado.</p>\';return}let r=await fetch(location.pathname+\'/list?book=\'+encodeURIComponent(name));let j=await r.json();contacts=j.contacts||[];render()}function render(){let q=(document.getElementById(\'q\').value||\'\').toLowerCase();let rows=contacts.filter(c=>JSON.stringify(c).toLowerCase().includes(q)).map(c=>`<tr><td>${esc(c.fullName)}</td><td>${esc(c.organization)}</td><td>${esc(c.email)}</td><td>${esc(c.phone)}</td><td>${esc(c.mobile)}</td><td><button onclick="editContact(\'${esc(c.file)}\')">Editar</button></td></tr>`).join(\'\');document.getElementById(\'list\').innerHTML=`<table><thead><tr><th>Nome</th><th>Empresa</th><th>E-mail</th><th>Telefone</th><th>Celular</th><th>Ações</th></tr></thead><tbody>${rows}</tbody></table>`}function editContact(file){current=contacts.find(c=>c.file===file);showEditor(current)}function newContact(){current={file:\'\',uid:\'\',fullName:\'\',firstName:\'\',lastName:\'\',organization:\'\',title:\'\',email:\'\',phone:\'\',mobile:\'\',note:\'\'};showEditor(current)}function field(k,l,type){return `<div><label>${l}</label><${type||\'input\'} id="f_${k}" value="${esc(current[k])}">${type===\'textarea\'?esc(current[k])+\'</textarea>\':\'\'}</div>`}function showEditor(c){current=c;document.getElementById(\'editor\').style.display=\'block\';document.getElementById(\'editor\').innerHTML=`<h2>${c.file?\'Editar contato\':\'Novo contato\'}</h2><div class="grid">${field(\'fullName\',\'Nome completo\')}${field(\'email\',\'E-mail\')}${field(\'firstName\',\'Primeiro nome\')}${field(\'lastName\',\'Sobrenome\')}${field(\'organization\',\'Empresa\')}${field(\'title\',\'Cargo\')}${field(\'phone\',\'Telefone\')}${field(\'mobile\',\'Celular\')}</div><label style="display:block;font-size:12px;font-weight:600;margin:8px 0 4px">Observações</label><textarea id="f_note" style="width:100%;height:80px">${esc(c.note)}</textarea><br><br><button onclick="saveContact()">Salvar</button> <button class="danger" onclick="deleteContact()" ${c.file?\'\':\'disabled\'}>Excluir</button> <button class="secondary" onclick="document.getElementById(\'editor\').style.display=\'none\'">Cancelar</button>`}async function saveContact(){let o=Object.assign({},current);[\'fullName\',\'email\',\'firstName\',\'lastName\',\'organization\',\'title\',\'phone\',\'mobile\',\'note\'].forEach(k=>o[k]=document.getElementById(\'f_\'+k).value);let r=await fetch(location.pathname+\'/save\',{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},body:JSON.stringify({book:document.getElementById(\'book\').value,contact:o})});if(!r.ok){alert(await r.text());return}document.getElementById(\'editor\').style.display=\'none\';await loadContacts()}async function deleteContact(){if(!current.file)return;let r=await fetch(location.pathname+\'/delete\',{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},body:JSON.stringify({book:document.getElementById(\'book\').value,file:current.file})});if(!r.ok){alert(await r.text());return}document.getElementById(\'editor\').style.display=\'none\';await loadContacts()}init();</script></body></html>'; }
  ...
  obj.server_startup=function(){ log('loaded 1.2.12'); };
