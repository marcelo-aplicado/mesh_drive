...
module.exports.meshdrive = function (parent) {
  ...
  obj.exports = ['onWebUIStartupEnd','goPageEnd','copyDetectedAddress','copyMapCommand','openMeshDriveAdmin','openMeshDriveContacts','injectMeshDriveLauncher'];
  ...
  function vcardUnfold(txt){ return String(txt||'').replace(/\r\n[ \t]/g,'').replace(/\n[ \t]/g,'').split(/\r?\n/); }
  function vcardValue(line){ const i=String(line||'').indexOf(':'); return i>=0?String(line).substring(i+1):''; }
  function vcardEscape(v){ return String(v||'').replace(/\\/g,'\\\\').replace(/\n/g,'\\n').replace(/;/g,'\\;').replace(/,/g,'\\,'); }
  function vcardUnescape(v){ return String(v||'').replace(/\\n/gi,'\n').replace(/\\,/g,',').replace(/\\;/g,';').replace(/\\\\/g,'\\'); }
  function parseVCard(txt,file){ const c={file:file||'',uid:'',fullName:'',firstName:'',lastName:'',organization:'',department:'',title:'',email:'',phone:'',mobile:'',note:''}; vcardUnfold(txt).forEach(l=>{ const u=String(l||'').toUpperCase(); if(u.indexOf('UID')===0)c.uid=vcardUnescape(vcardValue(l)); else if(u.indexOf('FN')===0)c.fullName=vcardUnescape(vcardValue(l)); else if(u.indexOf('N')===0){ const a=vcardValue(l).split(';'); c.lastName=vcardUnescape(a[0]||''); c.firstName=vcardUnescape(a[1]||''); } else if(u.indexOf('ORG')===0){ const a=vcardValue(l).split(';'); c.organization=vcardUnescape(a[0]||''); if(!c.department&&a[1])c.department=vcardUnescape(a[1]); } else if(u.indexOf('X-DEPARTMENT')===0)c.department=vcardUnescape(vcardValue(l)); else if(u.indexOf('TITLE')===0)c.title=vcardUnescape(vcardValue(l)); else if(u.indexOf('EMAIL')===0&&!c.email)c.email=vcardUnescape(vcardValue(l)); else if(u.indexOf('TEL')===0){ if((u.indexOf('CELL')>=0||u.indexOf('MOBILE')>=0) && !c.mobile)c.mobile=vcardUnescape(vcardValue(l)); else if(!c.phone)c.phone=vcardUnescape(vcardValue(l)); } else if(u.indexOf('NOTE')===0)c.note=vcardUnescape(vcardValue(l)); }); if(!c.fullName)c.fullName=contactDisplayName(c,file); return c; }
  function contactDisplayName(c,file){ const name=((c.firstName||'')+' '+(c.lastName||'')).trim() || c.fullName || c.email || c.phone || file || 'Contato'; return c.department ? (name+' ('+c.department+')') : name; }
  function contactFileName(c){ const base=safe((c.uid||contactDisplayName(c)||c.email||crypto.randomUUID()).replace(/\.vcf$/i,'')); return base+'.vcf'; }
  function buildVCard(c){ const uid=c.uid||crypto.randomUUID(); const fn=contactDisplayName(c); const lines=['BEGIN:VCARD','VERSION:3.0','UID:'+vcardEscape(uid),'FN:'+vcardEscape(fn),'N:'+vcardEscape(c.lastName||'')+';'+vcardEscape(c.firstName||'')+';;;']; if(c.organization||c.department)lines.push('ORG:'+vcardEscape(c.organization||'')+(c.department?';'+vcardEscape(c.department):'')); if(c.department)lines.push('X-DEPARTMENT:'+vcardEscape(c.department)); if(c.title)lines.push('TITLE:'+vcardEscape(c.title)); if(c.email)lines.push('EMAIL;TYPE=INTERNET:'+vcardEscape(c.email)); if(c.phone)lines.push('TEL;TYPE=WORK,VOICE:'+vcardEscape(c.phone)); if(c.mobile)lines.push('TEL;TYPE=CELL:'+vcardEscape(c.mobile)); if(c.note)lines.push('NOTE:'+vcardEscape(c.note)); lines.push('END:VCARD'); return lines.join('\r\n')+'\r\n'; }
  function contactsHtml(ctx){ return '<!doctype html><html><head><meta charset="utf-8"><title>Mesh Drive - Contatos</title><style>body{font-family:Segoe UI,Arial,sans-serif;margin:24px;background:#f6f8fa;color:#24292f}.card{background:white;border:1px solid #d0d7de;border-radius:10px;padding:18px;margin-bottom:14px}.top{display:flex;gap:8px;align-items:center;margin-bottom:14px}button{padding:8px 12px;border-radius:6px;border:1px solid #1f6feb;background:#1f6feb;color:white;cursor:pointer}.secondary{background:#57606a;border-color:#57606a}.danger{background:#cf222e;border-color:#cf222e}input,textarea,select{box-sizing:border-box;padding:8px;border:1px solid #d0d7de;border-radius:6px}table{width:100%;border-collapse:collapse;background:white;border:1px solid #d0d7de;border-radius:10px;overflow:hidden}th,td{padding:8px;border-bottom:1px solid #d0d7de;text-align:left;font-size:13px}th{background:#f6f8fa}.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.grid label{display:block;font-size:12px;font-weight:600;margin:6px 0 4px}.small{color:#57606a;font-size:13px}.readonly{background:#f6f8fa;color:#57606a}</style></head><body><h1>Mesh Drive - Contatos CardDAV</h1><p class="small">Tenant: <b>'+x(tenantKey(ctx))+'</b> | Nome completo gerado como Primeiro Nome + Sobrenome + (Setor).</p><div class="top"><select id="book"></select><input id="q" placeholder="Pesquisar..." style="flex:1"><button onclick="loadContacts()">Recarregar</button><button onclick="newContact()">Novo contato</button><button class="secondary" onclick="location.href=\'/meshdrive\'">Compartilhamentos</button></div><div id="list"></div><div id="editor" class="card" style="display:none"></div><script>let books=[],contacts=[],current=null;function esc(s){return String(s||\'\').replace(/[&<>\\"]/g,c=>({\'&\':\'&amp;\',\'<\':\'&lt;\',\'>\':\'&gt;\',\'"\':\'&quot;\'}[c]))}function calcFull(c){let n=((c.firstName||\'\')+\' \'+(c.lastName||\'\')).trim()||c.fullName||c.email||\'Contato\';return c.department?n+\' (\'+c.department+\')\':n}async function init(){let r=await fetch(location.pathname+\'/list\');let j=await r.json();books=j.books||[];let b=document.getElementById(\'book\');b.innerHTML=books.map(x=>\'<option>\'+esc(x.name)+\'</option>\').join(\'\');b.onchange=loadContacts;document.getElementById(\'q\').oninput=render;await loadContacts()}async function loadContacts(){let name=document.getElementById(\'book\').value||\'\';if(!name){document.getElementById(\'list\').innerHTML=\'<p>Nenhum compartilhamento CardDAV encontrado.</p>\';return}let r=await fetch(location.pathname+\'/list?book=\'+encodeURIComponent(name));let j=await r.json();contacts=j.contacts||[];render()}function render(){let q=(document.getElementById(\'q\').value||\'\').toLowerCase();let rows=contacts.filter(c=>JSON.stringify(c).toLowerCase().includes(q)).map(c=>\'<tr><td>\'+esc(calcFull(c))+\'</td><td>\'+esc(c.department)+\'</td><td>\'+esc(c.title)+\'</td><td>\'+esc(c.email)+\'</td><td>\'+esc(c.phone)+\'</td><td>\'+esc(c.mobile)+\'</td><td><button onclick="editContact(\\\'\'+esc(c.file)+\'\\\')">Editar</button></td></tr>\').join(\'\');document.getElementById(\'list\').innerHTML=\'<table><thead><tr><th>Nome</th><th>Setor</th><th>Cargo</th><th>E-mail</th><th>Telefone</th><th>Celular</th><th>Ações</th></tr></thead><tbody>\'+rows+\'</tbody></table>\'}function editContact(file){current=contacts.find(c=>c.file===file);showEditor(current)}function newContact(){current={file:\'\',uid:\'\',fullName:\'\',firstName:\'\',lastName:\'\',organization:\'\',department:\'\',title:\'\',email:\'\',phone:\'\',mobile:\'\',note:\'\'};showEditor(current)}function val(k){let e=document.getElementById(\'f_\'+k);return e?e.value:\'\'}function updatePreview(){let c={firstName:val(\'firstName\'),lastName:val(\'lastName\'),department:val(\'department\'),fullName:val(\'fullName\'),email:val(\'email\')};document.getElementById(\'f_fullName\').value=calcFull(c)}function input(k,l,ro){return \'<div><label>\'+l+\'</label><input \'+(ro?\'readonly class="readonly"\':\'\')+\' id="f_\'+k+\'" value="\'+esc(current[k])+\'"></div>\'}function showEditor(c){current=c||{};document.getElementById(\'editor\').style.display=\'block\';document.getElementById(\'editor\').innerHTML=\'<h2>\'+(current.file?\'Editar contato\':\'Novo contato\')+\'</h2><div class="grid">\'+input(\'firstName\',\'Primeiro nome\')+input(\'lastName\',\'Sobrenome\')+input(\'department\',\'Setor\')+input(\'fullName\',\'Nome completo gerado\',true)+input(\'organization\',\'Empresa\')+input(\'title\',\'Cargo\')+input(\'email\',\'E-mail\')+input(\'phone\',\'Telefone\')+input(\'mobile\',\'Celular\')+\'</div><label style="display:block;font-size:12px;font-weight:600;margin:8px 0 4px">Observações</label><textarea id="f_note" style="width:100%;height:80px">\'+esc(current.note)+\'</textarea><br><br><button onclick="saveContact()">Salvar</button> <button class="danger" onclick="deleteContact()" \'+(current.file?\'\':\'disabled\')+\'>Excluir</button> <button class="secondary" onclick="document.getElementById(\\\'editor\\\').style.display=\\\'none\\\'">Cancelar</button>\';[\'firstName\',\'lastName\',\'department\',\'email\'].forEach(k=>document.getElementById(\'f_\'+k).addEventListener(\'input\',updatePreview));updatePreview()}async function saveContact(){let o=Object.assign({},current);[\'firstName\',\'lastName\',\'department\',\'fullName\',\'organization\',\'title\',\'email\',\'phone\',\'mobile\',\'note\'].forEach(k=>o[k]=val(k));o.fullName=calcFull(o);let r=await fetch(location.pathname+\'/save\',{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},body:JSON.stringify({book:document.getElementById(\'book\').value,contact:o})});if(!r.ok){alert(await r.text());return}document.getElementById(\'editor\').style.display=\'none\';await loadContacts()}async function deleteContact(){if(!current.file)return;let r=await fetch(location.pathname+\'/delete\',{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},body:JSON.stringify({book:document.getElementById(\'book\').value,file:current.file})});if(!r.ok){alert(await r.text());return}document.getElementById(\'editor\').style.display=\'none\';await loadContacts()}init();</script></body></html>'; }
  async function adminHandler(req,res){ const u=await auth(req,res,true,false); if(!u)return; const ctx=u.domainContext,pathname=(req.url||'').split('?')[0].replace(/\/$/,''); if(pathname.endsWith('/contacts/list')){ const urlObj=new URL(req.url,'https://'+(req.headers.host||'localhost')); const qs=Object.fromEntries(urlObj.searchParams.entries()); const shares=allowedCardDavShares(u); if(!qs.book){ res.writeHead(200,{'Content-Type':'application/json;charset=utf-8'}); return res.end(JSON.stringify({books:shares.map(s=>({name:s.name,permission:s.permission}))},null,2)); } const share=findCardDavShareByName(u,qs.book); if(!share){res.writeHead(404);return res.end('Address book not found');} const root=shareRoot(u,share); const contacts=vcfFiles(root).map(f=>parseVCard(fs.readFileSync(path.join(root,f),'utf8'),f)); res.writeHead(200,{'Content-Type':'application/json;charset=utf-8'}); return res.end(JSON.stringify({contacts},null,2)); } if(pathname.endsWith('/contacts/save')){ try{ const body=JSON.parse(await readBody(req)); const share=findCardDavShareByName(u,body.book); if(!share){res.writeHead(404);return res.end('Address book not found');} if(share.permission!=='write'){res.writeHead(405);return res.end('Sem permissão de gravação');} const root=shareRoot(u,share); const c=body.contact||{}; const file=safePath(c.file||contactFileName(c)); if(!file.toLowerCase().endsWith('.vcf')){res.writeHead(415);return res.end('Arquivo inválido');} fs.writeFileSync(path.join(root,file),buildVCard(c)); res.writeHead(200,{'Content-Type':'application/json;charset=utf-8'}); return res.end(JSON.stringify({ok:true,file},null,2)); }catch(e){res.writeHead(400,{'Content-Type':'text/plain;charset=utf-8'});return res.end(String(e.message||e));} } if(pathname.endsWith('/contacts/delete')){ try{ const body=JSON.parse(await readBody(req)); const share=findCardDavShareByName(u,body.book); if(!share){res.writeHead(404);return res.end('Address book not found');} if(share.permission!=='write'){res.writeHead(405);return res.end('Sem permissão de gravação');} const root=shareRoot(u,share),file=safePath(body.file||''); const full=path.resolve(path.join(root,file)); if(full.indexOf(root+path.sep)!==0){res.writeHead(403);return res.end();} if(fs.existsSync(full))fs.rmSync(full,{force:true}); res.writeHead(200,{'Content-Type':'application/json;charset=utf-8'}); return res.end(JSON.stringify({ok:true},null,2)); }catch(e){res.writeHead(400,{'Content-Type':'text/plain;charset=utf-8'});return res.end(String(e.message||e));} } if(pathname.endsWith('/contacts')){ res.writeHead(200,{'Content-Type':'text/html;charset=utf-8'}); return res.end(contactsHtml(ctx)); } if(pathname.endsWith('/config')){ if((req.method||'GET').toUpperCase()==='GET'){ res.writeHead(200,{'Content-Type':'application/json;charset=utf-8'}); return res.end(JSON.stringify(readSharesConfig(ctx),null,2)); } if((req.method||'GET').toUpperCase()==='POST'){ try{ const saved=writeSharesConfig(ctx,JSON.parse(await readBody(req))); res.writeHead(200,{'Content-Type':'application/json;charset=utf-8'}); return res.end(JSON.stringify(saved,null,2)); }catch(e){ res.writeHead(400,{'Content-Type':'text/plain;charset=utf-8'}); return res.end(String(e.message||e)); } } } res.writeHead(200,{'Content-Type':'text/html;charset=utf-8'}); res.end(htmlPage(ctx)); }
  ...
  obj.server_startup=function(){ log('loaded 1.2.14'); };
