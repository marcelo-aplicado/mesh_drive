...
module.exports.meshdrive = function (parent) {
  ...
  function log(m){ if(!cfg.debug) return; try{ obj.debug('PLUGIN','Mesh Drive',m); }catch(e){} try{ console.log('PLUGIN: Mesh Drive: '+m); }catch(e){} }
  function err(m){ if(!cfg.debug) return; try{ console.warn('PLUGIN: Mesh Drive: '+m); }catch(e){} }
  ...
  function defaultShares(){ return {shares:[{name:'Public',path:'public',readUsers:['*'],writeUsers:[],readGroups:[],writeGroups:[],anonymousAccess:'read'},{name:'Contatos',path:'contacts',readUsers:['*'],writeUsers:[],readGroups:[],writeGroups:[],anonymousAccess:'read'}]}; }
  ...
  function readSharesConfig(ctx){ const f=ensureSharesFile(ctx); try{ const d=JSON.parse(fs.readFileSync(f,'utf8')); return (d&&Array.isArray(d.shares))?d:{shares:[]}; }catch(e){ err('shares parse error '+f); return {shares:[]}; } }
  ...
  function writeSharesConfig(ctx,data){
    ...
    const names={}, personal=safe(cfg.personalFolderName).toLowerCase();
    const clean={shares:data.shares.map(normalizeShare).filter(s=>s.name&&s.path).map(s=>{ const k=s.name.toLowerCase(); if(k===personal) throw new Error('Nome reservado: '+cfg.personalFolderName); if(names[k]) throw new Error('Nome duplicado: '+s.name); names[k]=true; return s; })};
    fs.writeFileSync(sharesFileForContext(ctx),JSON.stringify(clean,null,2));
    return clean;
  }
  ...
  function driveTarget(u,rel){ const parts=rel.split('/').filter(Boolean),personal=safe(cfg.personalFolderName); if(parts.length===0)return {kind:'root',rel:'/'}; if(!u.anonymous&&parts[0].toLowerCase()===personal.toLowerCase()){ const root=userRoot(u),inside=parts.slice(1).join('/'),p=path.resolve(path.join(root,safePath(inside))); if(p!==root&&p.indexOf(root+path.sep)!==0)return null; return {kind:'personal',root,path:p,rel:'/'+parts.join('/'),readOnly:cfg.readOnly}; } const share=findShareByName(u,parts[0]); if(!share)return null; const root=shareRoot(u,share),inside=parts.slice(1).join('/'),p=path.resolve(path.join(root,safePath(inside))); if(p!==root&&p.indexOf(root+path.sep)!==0)return null; return {kind:'share',share,root,path:p,rel:'/'+parts.join('/'),readOnly:share.permission!=='write'}; }
  ...
  function handleRealDav(req,res,target,u){
  ...
  async function driveDav(req,res){ const u=await auth(req,res,false,true); if(!u)return; const method=(req.method||'GET').toUpperCase(); if(method==='OPTIONS')return sendOptions(res,false); const rel=routePath(req,cfg.route),target=driveTarget(u,rel); if(!target){res.writeHead(404);return res.end();} try{ if(target.kind==='root'){ if(method==='PROPFIND'){ let out=propVirtual('drive','/'); const depth=req.headers.depth||'1'; if(depth!=='0'){ if(!u.anonymous)out+=propVirtual(cfg.personalFolderName,'/'+cfg.personalFolderName); allowedShares(u).forEach(s=>{out+=propVirtual(s.name,'/'+s.name);}); } return xml(res,207,'<?xml version="1.0" encoding="utf-8"?><D:multistatus xmlns:D="DAV:">'+out+'</D:multistatus>'); } res.writeHead(403); return res.end(); } return handleRealDav(req,res,target,u); }catch(e){err('drive error'); try{res.writeHead(500);res.end();}catch(ex){} } }
  ...
  function vcfFiles(root){ try{return fs.readdirSync(root).filter(f=>f.toLowerCase().endsWith('.vcf')).slice(0,cfg.maxCardDavItems);}catch(e){return[];} }
  ...
  async function carddav(req,res){ const u=await auth(req,res,false,true); if(!u)return; const method=(req.method||'GET').toUpperCase(); if(method==='OPTIONS')return sendOptions(res,true); const rel=routePath(req,cfg.carddavRoute),parts=rel.split('/').filter(Boolean),baseUser=norm(u.username||'anonymous'); try{
    ...
    if(method==='REPORT'){
      if(!(parts[0]==='addressbooks'&&parts[1]===baseUser&&parts[2])){res.writeHead(404);return res.end();} const share=findShareByName(u,parts[2]); if(!share){res.writeHead(404);return res.end();} const root=shareRoot(u,share); const body=await readBody(req); let out='', hrefs=[...body.matchAll(/<[^:>]*:?href[^>]*>([^<]+)<\/[^:>]*:?href>/gi)].map(m=>decodeURIComponent(m[1])).slice(0,cfg.maxCardDavItems); if(hrefs.length){hrefs.forEach(h=>{const f=path.basename(h),full=path.join(root,safePath(f)); if(fs.existsSync(full))out+=cardResp(cardHref('/addressbooks/'+baseUser+'/'+share.name+'/'+f),cardFileProps(f,full)+cardData(full));});} else {vcfFiles(root).forEach(f=>{const full=path.join(root,f); out+=cardResp(cardHref('/addressbooks/'+baseUser+'/'+share.name+'/'+f),cardFileProps(f,full)+cardData(full));});} return cardMulti(res,out);
    }
  ...
  obj.hook_setupHttpHandlers=function(){
    ...
    const key='__meshdrive_handlers_registered__';
    if(global[key]){ log('handlers already registered, skipping'); return; }
    const a=app(); if(!a){ err('Express app not found'); return; }
    global[key]=true;
    mkdir(pluginDir); mkdir(rootDomainForFolder(cfg.meshDomainFolder||'domain'));
    a.use(cfg.route,(req,res)=>driveDav(req,res));
    a.use(cfg.carddavRoute,(req,res)=>carddav(req,res));
    a.use(cfg.adminRoute,(req,res)=>adminHandler(req,res));
    log('handlers registered once');
  };
  obj.server_startup=function(){ log('loaded 2.4.2-test quiet mode'); };
