...
module.exports.meshdrive = function (parent) {
    ...
    function userRoot(u) { var folder = (u && u.domainContext && u.domainContext.folder) || cfg.meshDomainFolder || 'domain'; var r = path.join(rootDomainForFolder(folder), cfg.userFolderPrefix + norm(u.username || u.id || 'user')); if (cfg.defaultUserSubFolder) r = path.join(r, safe(cfg.defaultUserSubFolder)); mkdir(r); return path.resolve(r); }
    ...
    async function dav(req, res) {
        ...
        var rel = reqPath(req), fp = full(u, rel); if (!fp) { res.writeHead(403); return res.end(); }
        try {
            switch ((req.method || 'GET').toUpperCase()) {
                ...
                case 'PROPFIND': { if (!fs.existsSync(fp)) { res.writeHead(404); return res.end(); } var depth=req.headers.depth||'1', out=prop(fp,rel), st=fs.statSync(fp); if (depth!=='0' && st.isDirectory()) fs.readdirSync(fp).forEach(function(n){ out += prop(path.join(fp,n), path.posix.join(rel,n)); }); xml(res,207,'<?xml version="1.0" encoding="utf-8"?><D:multistatus xmlns:D="DAV:">' + out + '</D:multistatus>'); break; }
