...
module.exports.meshdrive = function (parent) {
    ...
    var cfg = Object.assign({
        ...
        hostDomainMap: {},
        sharedFolderName: 'Compartilhado',
    ...
    function ensureSharesFile() {
        if (!fs.existsSync(sharesFile)) {
            mkdir(pluginDir);
            fs.writeFileSync(sharesFile, JSON.stringify({ shares: [] }, null, 2));
        }
    }
    function readSharesConfig() {
        ensureSharesFile();
        try {
            var data = JSON.parse(fs.readFileSync(sharesFile, 'utf8'));
            if (!data || !Array.isArray(data.shares)) return { shares: [] };
            return data;
        } catch (e) { log('shares.json parse error: ' + e); return { shares: [] }; }
    }
    function writeSharesConfig(data) {
        if (!data || !Array.isArray(data.shares)) throw new Error('shares must be an array');
        var clean = { shares: data.shares.map(function(s) {
            return {
                name: safe(s.name || 'Share'),
                path: safePath(s.path || ('shares/' + safe(s.name || 'Share'))),
                access: String(s.access || 'read').toLowerCase() === 'write' ? 'write' : 'read',
                users: Array.isArray(s.users) ? s.users.map(String) : [],
    ...
    function resolveDomainContext(req) {
        ...
        var domainId = null, source = 'default';
        if (mapped) { domainId = String(mapped).toLowerCase(); source = 'hostDomainMap'; }
        if (domainId === null) { domainId = findDomainIdByHost(host); if (domainId !== null) source = 'meshConfig.domains'; }
        var candidates = [];
        ...
        return { host: host, id: domainIdFromFolder(folder), folder: folder, source: source };
    }
    ...
    function nativeResultOk(result) { if (result === true) return true; if (result === false || result == null) return false; if (typeof result === 'string') return ['ok','true'].indexOf(result.toLowerCase()) >= 0; if (typeof result === 'object') return !!(result.ok || result.success || result.valid || result.authenticated || result._id || result.userid || result.user || result.name); return false; }
    function getContainerPath(name) { if (name === 'webserver') return obj.meshServer && obj.meshServer.webserver; if (name === 'meshServer') return obj.meshServer; if (name === 'parent') return parent; if (name === 'parentParent') return parent && parent.parent; return null; }
    function callNativeFunction(fn, args) { return new Promise(function(resolve) { var done = false; function finish(v) { if (!done) { done = true; resolve(v); } } var timeout = setTimeout(function() { finish({ status: 'timeout' }); }, 300); var cb = function(a, b) { clearTimeout(timeout); if (arguments.length > 1) finish({ status: 'callback', result: b }); else finish({ status: 'callback', result: a }); }; try { var r = fn.apply(null, args.concat([cb])); if (r && typeof r.then === 'function') r.then(function(v) { clearTimeout(timeout); finish({ status: 'promise', result: v }); }).catch(function(e) { clearTimeout(timeout); finish({ status: 'promise-error', result: e }); }); else if (r !== undefined) { clearTimeout(timeout); finish({ status: 'return', result: r }); } } catch (e) { clearTimeout(timeout); finish({ status: 'throw', result: e }); } }); }
    async function tryNativeMeshAuth(username, password, ctx, userDoc) {
        var methods = ['validateUser','ValidateUser','validateUserPassword','verifyUserPassword','checkUserPassword','checkUserPass','authenticateUser','authUser','loginUser','checkUserLogin','validateLogin'];
        var containers = ['webserver','meshServer','parent','parentParent'];
        var user = norm(username), domainId = (ctx && ctx.id) || '', userId = (userDoc && userDoc._id) || (domainId ? ('user/' + domainId + '/' + user) : ('user//' + user));
        for (var ci = 0; ci < containers.length; ci++) { var c = getContainerPath(containers[ci]); if (!c) continue; for (var mi = 0; mi < methods.length; mi++) { var fn = c[methods[mi]]; if (typeof fn !== 'function') continue; var variants = [[domainId,user,password],[user,password,domainId],[userId,password],[userDoc,password],[{domain:domainId,userid:userId,username:user,user:userDoc,password:password}],[user,password]]; for (var vi = 0; vi < variants.length; vi++) { var r = await callNativeFunction(fn, variants[vi]); if (nativeResultOk(r.result)) return true; } } }
        return null;
    }
    async function findUser(username, ctx) {
        ...
        for (var i = 0; i < ids.length; i++) { var d = await dbGet(ids[i]); if (d) return { id: ids[i], doc: d, username: u, domainContext: ctx }; }
    ...
    async function validate(username, password, ctx) {
        ...
        if (!d || d.locked || d.siteadmin === -1) return null;
        var nativeOk = await tryNativeMeshAuth(username, password, ctx, d);
        if (nativeOk === true) return { id: d._id || f.id, username: f.username, doc: d, domainContext: ctx };
    ...
    function isWriteMethod(method) { return ['PUT','DELETE','MOVE','COPY','MKCOL','PROPPATCH'].indexOf(String(method || '').toUpperCase()) >= 0; }
    ...
    function shareAccessForUser(share, u) {
        var ids = userIdentifiers(u), gids = userGroupIdentifiers(u), access = String(share.access || 'read').toLowerCase() === 'write' ? 'write' : 'read';
        var users = Array.isArray(share.users) ? share.users.map(String) : [];
        var groups = Array.isArray(share.groups) ? share.groups.map(String) : [];
        if (users.indexOf('*') >= 0) return access;
        for (var i = 0; i < users.length; i++) { if (ids.indexOf(String(users[i]).toLowerCase()) >= 0 || ids.indexOf(norm(users[i])) >= 0) return access; }
        for (var j = 0; j < groups.length; j++) { if (gids.indexOf(String(groups[j]).toLowerCase()) >= 0 || gids.indexOf(norm(groups[j])) >= 0) return access; }
        return null;
    }
    ...
    function shareRoot(u, share) { var root = tenantRoot(u), rel = safePath(share.path || ('shares/' + share.name)); var full = path.resolve(path.join(root, rel)); if (full !== root && full.indexOf(root + path.sep) !== 0) return null; mkdir(full); return full; }
    function resolveTarget(u, rel) {
        var parts = rel.split('/').filter(Boolean), sharedName = cfg.sharedFolderName || 'Compartilhado';
        if (parts.length > 0 && parts[0].toLowerCase() === sharedName.toLowerCase()) {
            if (parts.length === 1) return { kind: 'sharesRoot', rel: '/' + sharedName };
            var share = findShareByName(u, parts[1]); if (!share) return null;
            var root = shareRoot(u, share); if (!root) return null;
            var inside = parts.slice(2).join('/'), p = path.resolve(path.join(root, safePath(inside)));
            if (p !== root && p.indexOf(root + path.sep) !== 0) return null;
            return { kind: 'share', share: share, root: root, path: p, rel: '/' + parts.join('/'), readOnly: String(share.access).toLowerCase() !== 'write' };
        }
        var ur = userRoot(u), clean = path.normalize('/' + rel).replace(/^([/\\])+/, ''), p2 = path.resolve(path.join(ur, clean));
        if (p2 !== ur && p2.indexOf(ur + path.sep) !== 0) return null;
        return { kind: 'personal', root: ur, path: p2, rel: rel || '/', readOnly: cfg.readOnly };
    }
    ...
    function propReal(f, rel) { var st = fs.statSync(f), isD = st.isDirectory(), display = path.basename(f) || '/'; return '<D:response><D:href>' + x(href(rel + (isD && !rel.endsWith('/') ? '/' : ''))) + '</D:href><D:propstat><D:prop><D:displayname>' + x(display) + '</D:displayname><D:getlastmodified>' + st.mtime.toUTCString() + '</D:getlastmodified><D:creationdate>' + st.birthtime.toISOString() + '</D:creationdate>' + (isD ? '<D:resourcetype><D:collection/></D:resourcetype>' : '<D:resourcetype/>') + (!isD ? '<D:getcontentlength>' + st.size + '</D:getcontentlength>' : '') + '<D:getetag>"' + st.size + '-' + Number(st.mtimeMs).toString(16) + '"</D:getetag></D:prop><D:status>HTTP/1.1 200 OK</D:status></D:propstat></D:response>'; }
    function propVirtual(name, rel) { return '<D:response><D:href>' + x(href(rel.endsWith('/') ? rel : rel + '/')) + '</D:href><D:propstat><D:prop><D:displayname>' + x(name) + '</D:displayname><D:resourcetype><D:collection/></D:resourcetype></D:prop><D:status>HTTP/1.1 200 OK</D:status></D:propstat></D:response>'; }
    ...
    async function dav(req, res) {
        var u = await auth(req, res, false); if (!u) return;
        var rel = reqPath(req), target = resolveTarget(u, rel);
        ...
        try {
            ...
            if (isWriteMethod(method) && target.readOnly) { res.writeHead(405); return res.end(); }
            if (method === 'PROPFIND') {
                var depth = req.headers.depth || '1', out = '';
                if (target.kind === 'sharesRoot') { out += propVirtual(cfg.sharedFolderName, '/' + cfg.sharedFolderName); if (depth !== '0') allowedShares(u).forEach(function(s) { out += propVirtual(s.name, '/' + cfg.sharedFolderName + '/' + s.name); }); return xml(res, 207, '<?xml version="1.0" encoding="utf-8"?><D:multistatus xmlns:D="DAV:">' + out + '</D:multistatus>'); }
                if (!fs.existsSync(target.path)) { res.writeHead(404); return res.end(); }
                out += propReal(target.path, rel);
                var st = fs.statSync(target.path);
                if (depth !== '0' && st.isDirectory()) fs.readdirSync(target.path).forEach(function(n) { out += propReal(path.join(target.path, n), path.posix.join(rel, n)); });
                if (rel === '/' || rel === '') out += propVirtual(cfg.sharedFolderName, '/' + cfg.sharedFolderName);
                return xml(res, 207, '<?xml version="1.0" encoding="utf-8"?><D:multistatus xmlns:D="DAV:">' + out + '</D:multistatus>');
            }
            if (method === 'GET' || method === 'HEAD') { if (!target.path || !fs.existsSync(target.path)) { res.writeHead(404); return res.end(); } var st2 = fs.statSync(target.path); if (st2.isDirectory()) { res.writeHead(403); return res.end(); } res.writeHead(200, { 'Content-Length': st2.size }); if (method === 'HEAD') res.end(); else fs.createReadStream(target.path).pipe(res); return; }
            if (method === 'PUT') { mkdir(path.dirname(target.path)); req.pipe(fs.createWriteStream(target.path)).on('finish', function() { res.writeHead(201); res.end(); }); return; }
            if (method === 'MKCOL') { if (fs.existsSync(target.path)) { res.writeHead(405); return res.end(); } mkdir(target.path); res.writeHead(201); return res.end(); }
            if (method === 'DELETE') { if (!fs.existsSync(target.path)) { res.writeHead(404); return res.end(); } fs.rmSync(target.path, { recursive: true, force: true }); res.writeHead(204); return res.end(); }
            if (method === 'MOVE' || method === 'COPY') { var dh = req.headers.destination; if (!dh) { res.writeHead(400); return res.end(); } var du = new URL(dh, cfg.publicUrl || ('https://' + (req.headers.host || 'localhost') + cfg.route)); var dr = decodeURIComponent(du.pathname); if (dr.indexOf(cfg.route) === 0) dr = dr.substring(cfg.route.length) || '/'; var dest = resolveTarget(u, dr); if (!dest || dest.readOnly || !dest.path) { res.writeHead(403); return res.end(); } mkdir(path.dirname(dest.path)); if (method === 'MOVE') fs.renameSync(target.path, dest.path); else copyRec(target.path, dest.path); res.writeHead(201); return res.end(); }
            if (method === 'LOCK') { var token = 'opaquelocktoken:' + crypto.randomUUID(); return xml(res, 200, '<?xml version="1.0" encoding="utf-8"?><D:prop xmlns:D="DAV:"><D:lockdiscovery><D:activelock><D:locktype><D:write/></D:locktype><D:lockscope><D:exclusive/></D:lockscope><D:depth>infinity</D:depth><D:owner>Mesh Drive</D:owner><D:timeout>Second-3600</D:timeout><D:locktoken><D:href>' + token + '</D:href></D:locktoken></D:activelock></D:lockdiscovery></D:prop>', { 'Lock-Token': '<' + token + '>' }); }
            if (method === 'UNLOCK') { res.writeHead(204); return res.end(); }
            if (method === 'PROPPATCH') { return xml(res, 207, '<?xml version="1.0" encoding="utf-8"?><D:multistatus xmlns:D="DAV:"><D:response><D:href>' + x(href(rel)) + '</D:href><D:propstat><D:prop/><D:status>HTTP/1.1 200 OK</D:status></D:propstat></D:response></D:multistatus>'); }
            res.writeHead(405); res.end();
        } catch (e) { log('handler error: ' + (e.stack || e)); try { res.writeHead(500); res.end(); } catch (ex) {} }
    }

    function htmlPage() {
        return '<!doctype html><html><head><meta charset="utf-8"><title>Mesh Drive - Compartilhamentos</title><style>body{font-family:Segoe UI,Arial,sans-serif;margin:24px;background:#f6f8fa;color:#24292f}h1{margin-top:0}.card{background:white;border:1px solid #d0d7de;border-radius:10px;padding:18px;margin-bottom:14px;box-shadow:0 1px 2px rgba(0,0,0,.04)}label{display:block;font-weight:600;margin:8px 0 4px}input,select,textarea{width:100%;box-sizing:border-box;padding:8px;border:1px solid #d0d7de;border-radius:6px}button{padding:8px 12px;border-radius:6px;border:1px solid #1f6feb;background:#1f6feb;color:white;cursor:pointer;margin-right:6px}.danger{background:#cf222e;border-color:#cf222e}.secondary{background:#57606a;border-color:#57606a}.row{display:grid;grid-template-columns:1fr 1fr 130px;gap:10px}.small{color:#57606a;font-size:13px}</style></head><body><h1>Mesh Drive - Compartilhamentos</h1><p class="small">Configuração salva em <code>plugins/meshdrive/shares.json</code>.</p><div id="list"></div><button onclick="addShare()">Adicionar compartilhamento</button><button class="secondary" onclick="load()">Recarregar</button><script>let data={shares:[]};function esc(s){return String(s||\'\').replace(/[&<>\"]/g,c=>({\'&\':\'&amp;\',\'<\':\'&lt;\',\'>\':\'&gt;\',\'"\':\'&quot;\'}[c]))}async function load(){const r=await fetch(location.pathname+\'/config\');data=await r.json();render()}function render(){const el=document.getElementById(\'list\');el.innerHTML=\'\';(data.shares||[]).forEach((s,i)=>{const d=document.createElement(\'div\');d.className=\'card\';d.innerHTML=`<div class="row"><div><label>Nome</label><input value="${esc(s.name)}" onchange="data.shares[${i}].name=this.value"></div><div><label>Caminho</label><input value="${esc(s.path)}" onchange="data.shares[${i}].path=this.value"></div><div><label>Permissão</label><select onchange="data.shares[${i}].access=this.value"><option value="read" ${s.access!==\'write\'?\'selected\':\'\'}>Leitura</option><option value="write" ${s.access===\'write\'?\'selected\':\'\'}>Gravação</option></select></div></div><label>Usuários (um por linha, use * para todos)</label><textarea rows="3" onchange="data.shares[${i}].users=this.value.split(/\\n|,/).map(x=>x.trim()).filter(Boolean)">${esc((s.users||[]).join(\'\\n\'))}</textarea><label>Grupos (um por linha)</label><textarea rows="2" onchange="data.shares[${i}].groups=this.value.split(/\\n|,/).map(x=>x.trim()).filter(Boolean)">${esc((s.groups||[]).join(\'\\n\'))}</textarea><br><br><button onclick="save()">Salvar</button><button class="danger" onclick="removeShare(${i})">Remover</button>`;el.appendChild(d)})}function addShare(){data.shares.push({name:\'Novo\',path:\'shares/novo\',access:\'read\',users:[],groups:[]});render()}function removeShare(i){data.shares.splice(i,1);render()}async function save(){const r=await fetch(location.pathname+\'/config\',{method:\'POST\',headers:{\'Content-Type\':\'application/json\'},body:JSON.stringify(data)});if(!r.ok){alert(await r.text());return}data=await r.json();render();alert(\'Configuração salva.\')}load();</script></body></html>';
    }
    ...
    async function adminHandler(req, res) {
        var u = await auth(req, res, true); if (!u) return;
        var pathname = (req.url || '').split('?')[0].replace(/\/$/, '');
        if (pathname.endsWith('/config')) {
            if ((req.method || 'GET').toUpperCase() === 'GET') { res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' }); return res.end(JSON.stringify(readSharesConfig(), null, 2)); }
            if ((req.method || 'GET').toUpperCase() === 'POST') { try { var body = await readBody(req); var saved = writeSharesConfig(JSON.parse(body)); res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' }); return res.end(JSON.stringify(saved, null, 2)); } catch(e) { res.writeHead(400, { 'Content-Type': 'text/plain; charset=utf-8' }); return res.end(String(e.message || e)); } }
        }
        res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' }); res.end(htmlPage());
    }
    ...
    obj.hook_setupHttpHandlers = function() { if (cfg.enabled === false) return; var a = app(); if (!a) { log('Express app not found'); return; } ensureSharesFile(); mkdir(rootDomainForFolder(cfg.meshDomainFolder || 'domain')); a.use(cfg.route, function(req, res) { dav(req, res); }); a.use(cfg.adminRoute, function(req, res) { adminHandler(req, res); }); log('registered route ' + cfg.route + ' and admin ' + cfg.adminRoute); };
